...
double l_infi (double e[501][501])
{
  double max = -1;
  for(int i=0; i<=n; i++)
    for(int j=0; j<=n; j++)
...
void residual ()
{
  ...
  while (!(row_num == n && col_num == n+1))
  {
    ...
    e[row_num][col_num] = pow((row_num*h),2) + pow((col_num*k),2) - x;
    col_num++;
  }
  infile.close();
  cout<<l1(e)<<endl;
